...
- Curated Turkish live channel catalog expanded and corrected in `LIVE_TV_CHANNEL_SEEDS`: verified TRT CDN channels now include TRT Haber, TRT 1, TRT 2, TRT Belgesel, TRT Muzik, TRT Spor, TRT Cocuk, TRT Genc, TRT Avaz, TRT Turk, plus Bloomberg HT, Haberturk, TGRT Haber, DHA, and Dream Turk. ATV, Kanal D, Star TV, and TV8 remain defined but `is_active=False` because their official browser-playable source is still not legitimately available; NTV stays available through its official YouTube simulcast only when a real live broadcast is present.
...
- Real XMLTV EPG mapping wired for the Turkish expansion via the existing `epgshare01.online` `TR3` region feed, with corrected populated HD feed IDs where the plain IDs were empty (`TRT.HABER.HD.tr`, `TRT1.HD.tr`, `NTV.HD.tr`, `CNN.TÜRK.HD.tr`, `SHOW.TV.HD.tr`, etc.). Channels with no genuine EPG match (for example `dha-tv`) intentionally have no `epg_channel_id` and correctly surface `Schedule unavailable` instead of invented data.
...
- Recovery follow-up: My Channel now persists an explicitly accepted plan per user/date (`is_accepted`, `accepted_at`) instead of relying on the latest generated lineup implicitly. The backend exposes `GET /api/my-channel/active` and `POST /api/my-channel/{plan_id}/accept`, and enforces the decision to keep only one active accepted plan per date.
- Recovery follow-up: the My Channel custom window flow now supports explicit start/end times on the frontend, and Live TV expands its EPG fetch window to cover the accepted plan for today so accepted live items remain highlighted after refresh/login.
